...
- **20차 확장 완료 — 실사용 피드백 12건 반영(버그 3 + 발견성 1 + 기능 5 + UI 2)**: 친구에게
  ...
  - **실기기/수동 테스트 필요(자동 빌드 검증만으로는 확인 불가)**: AI 채팅 조리순서/타이머 자연어
    ...
    사진 업로드 실제 동작(0020 마이그레이션을 Supabase SQL Editor에서 먼저 실행해야 함 — 아직
    미실행), 알러지 온보딩 단계 실제 가입 플로우, 이미지 생성 저장 시점 변경, 태그 발견성/설명
    문구가 실제로 이해하기 쉬운지, "오늘 만들었어요" 완료 화면 사진 업로드/대표이미지 반영,
    3구간 재료 목록/인분 조절 위치가 실기기 화면에서 자연스러운지. 회귀 테스트(레시피 저장/수정/
    삭제, 유튜브 변환, 장보기 담기 유지, 다크모드 토글)도 사용자 확인 필요.
