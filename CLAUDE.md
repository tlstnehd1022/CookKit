...
- 자막이 실제로 있는 영상은 `youtube-transcript` + Vercel Serverless Function 조합으로 자동 추출
  ...
  에러 메시지 제공).
- 다만 **자막이 아예 없는 영상**은 지금 구조로는 근본적으로 지원 불가 — 이 방식은 유튜브에 이미 올라가
  있는 자막 트랙을 가져오는 것일 뿐, 자막 자체가 없으면 가져올 대상이 없음.
- 이런 영상까지 커버하려면 영상의 음성을 직접 다운로드해서 STT(Whisper 등)를 돌려야 하는데, 이건
  오디오 다운로드+변환+STT API 호출까지 필요한 무거운 처리라 태생적으로 서버(그것도 지금의 "자막 프록시"
  수준이 아니라 실제 연산이 필요한 백엔드) 없이는 불가능함. 지금은 클라이언트 단독 + 최소 프록시 정도의
  아키텍처라서 범위 밖으로 판단, 나중에 다중 사용자 등으로 진짜 백엔드를 도입할 때 다시 검토하기로 함.
- 그때까지 임시 대응: 자막이 없는 영상은 명확한 에러 메시지를 보여주고, 사용자가 상단 대화창에서
  텍스트로 직접 설명해 레시피를 만드는 기존 기능으로 유도함.
