  ...
  items: {
    ...
    properties: {
      ...
      guessedName: {
        type: 'STRING',
        description:
          '일반적으로 통용되는 식료품 재료 이름으로 정규화한 이름. 축약/오타가 있으면 최대한 실제 의미로 ' +
          '해석해줘(예: "무G부침" → "무"). 브랜드명·제품 라인명·마케팅 문구는 항상 제거하고 일반 재료명만 ' +
          '남겨줘 — 같은 재료라도 브랜드가 다르면 서로 다른 문자열이 되어 재료가 중복 등록되기 때문에 ' +
          '반드시 브랜드 없는 이름으로 통일해야 해. 예: "풀무원 우리 두부" → "두부", "청정원 건강한 ' +
          '어간장" → "어간장", "CJ 백설 설탕" → "설탕". 원본(브랜드 포함)은 rawText에 그대로 남기니 ' +
          'guessedName에서는 지워도 정보가 사라지지 않아.',
      },
...
- guessedName은 브랜드명/제품 라인명/마케팅 문구를 반드시 제거하고 일반 재료명만 남겨줘. 예: "풀무원 우리 두부" → "두부", "청정원 건강한 어간장" → "어간장", "CJ 백설 설탕" → "설탕". rawText에는 원문을 그대로 남기니 guessedName은 항상 브랜드 없는 이름으로 통일해줘(브랜드가 달라도 같은 재료면 guessedName은 같아야 함).
